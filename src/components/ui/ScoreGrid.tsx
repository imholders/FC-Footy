  ...
  return (
    <React.Fragment key={`row-${rowIdx}`}>
      ...
      {Array.from({ length: 5 }, (_, colIdx) => {
        ...
        // Check if this cell represents the current score
        const isCurrentScore = rowVal === awayScore && colVal === homeScore;

        // Check if the square is OUT of the game (lower than the current score)
        const isOutOfGame = rowVal < awayScore || colVal < homeScore;
        ...
        return (
          <div
            ...
            className={`aspect-square p-1 flex items-center justify-center border border-lightPurple rounded transition-all duration-200 
              ${isCurrentScore ? 'bg-limeGreenOpacity' : ''} 
              ...
              <a href={`https://warpcast.com/~/profiles/${currentTicket.owner}`} target="_blank" rel="noopener noreferrer">
                <img
                  ...
                  className={`w-8 h-8 rounded-full 
                    ${isOutOfGame ? 'border-2 border-fontRed' : ''} 
                    `} // Apply both styles properly
                />
              </a>
            )}
          </div>
