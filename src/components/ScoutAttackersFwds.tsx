...
const ScoutAttackersFwds: React.FC<ScoutAttackersFwdsProps> = ({ playersIn }) => {
  ...
  const handleCastClick = (player: Players, rank: number) => {
    ...
    const url = `https://warpcast.com/~/compose?text=${encodedSummary}&channelKey=football&embeds[]=${BASE_URL}?tab=scout%20Players&embeds[]=${BASE_URL}&embeds[]=https://resources.premierleague.com/premierleague/photos/players/250x250/p${player.photo.replace(/\.[^/.]+$/, '.png')}`;
    console.log(url);
